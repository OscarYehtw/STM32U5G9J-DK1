...
static uint32_t IsI2c6MspCbValid = 0;
...
static uint32_t      I2c6InitCounter = 0;
...
/** @defgroup STM32U5x9J_DISCOVERY_BUS_Private_FunctionPrototypes BUS Private FunctionPrototypes
  * @{
  */
static void     I2C6_MspInit(I2C_HandleTypeDef *hI2c);
static void     I2C6_MspDeInit(I2C_HandleTypeDef *hI2c);
static int32_t  I2C6_WriteReg(uint16_t DevAddr, uint16_t MemAddSize, uint16_t Reg, uint8_t *pData, uint16_t Length);
static int32_t  I2C6_ReadReg(uint16_t DevAddr, uint16_t MemAddSize, uint16_t Reg, uint8_t *pData, uint16_t Length);
static int32_t  I2C6_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length);
static int32_t  I2C6_Send(uint16_t DevAddr, uint8_t *pData, uint16_t Length);
...
int32_t BSP_I2C6_Init(void)
{
  ...
  hbus_i2c6.Instance = BUS_I2C6;

  if (I2c6InitCounter == 0U)
  {
    I2c6InitCounter++;
    ...
    if (HAL_I2C_GetState(&hbus_i2c6) == HAL_I2C_STATE_RESET)
    {
...
#if (USE_HAL_I2C_REGISTER_CALLBACKS == 0)
      /* Init the I2C6 Msp */
      I2C6_MspInit(&hbus_i2c6);
#else
      if (IsI2c6MspCbValid == 0U)
      {
        if (BSP_I2C6_RegisterDefaultMspCallbacks() != BSP_ERROR_NONE)
        {
      ...
      if (MX_I2C6_Init(&hbus_i2c6, I2C_GetTiming(HAL_RCC_GetPCLK1Freq(), BUS_I2C6_FREQUENCY)) != HAL_OK)
      {
...
int32_t BSP_I2C6_DeInit(void)
{
  ...
  {
#if (USE_HAL_I2C_REGISTER_CALLBACKS == 0)
    I2C6_MspDeInit(&hbus_i2c6);
#endif /* (USE_HAL_I2C_REGISTER_CALLBACKS == 0) */

    /* Init the I2C */
    if (HAL_I2C_DeInit(&hbus_i2c6) != HAL_OK)
    {
      ret = BSP_ERROR_BUS_FAILURE;
    }
  }
...
#endif /* BSP_USE_CMSIS_OS */
  if (I2C6_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
  {
    if (HAL_I2C_GetError(&hbus_i2c6) == HAL_I2C_ERROR_AF)
    {
...
int32_t BSP_I2C6_ReadReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (I2C6_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
int32_t BSP_I2C6_WriteReg16(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (I2C6_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
int32_t BSP_I2C6_ReadReg16(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (I2C6_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
int32_t BSP_I2C6_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (I2C6_Recv(DevAddr, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
int32_t BSP_I2C6_Send(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (I2C6_Send(DevAddr, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
int32_t BSP_I2C6_IsReady(uint16_t DevAddr, uint32_t Trials)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (HAL_I2C_IsDeviceReady(&hbus_i2c6, DevAddr, Trials, 1000) != HAL_OK)
  {
    ret = BSP_ERROR_BUSY;
  }
#if defined(BSP_USE_CMSIS_OS)
...
#if (USE_HAL_I2C_REGISTER_CALLBACKS > 0)
/**
  * @brief Register Default I2C6 Bus Msp Callbacks
  * @retval BSP status
  */
int32_t BSP_I2C6_RegisterDefaultMspCallbacks(void)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  __HAL_I2C_RESET_HANDLE_STATE(&hbus_i2c6);

  /* Register default MspInit/MspDeInit Callback */
  if (HAL_I2C_RegisterCallback(&hbus_i2c6, HAL_I2C_MSPINIT_CB_ID, I2C6_MspInit) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else if (HAL_I2C_RegisterCallback(&hbus_i2c6, HAL_I2C_MSPDEINIT_CB_ID, I2C6_MspDeInit) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else
...
/**
  * @brief Register I2C6 Bus Msp Callback registering
  * @param Callbacks     pointer to I2C6 MspInit/MspDeInit callback functions
  * @retval BSP status
  */
int32_t BSP_I2C6_RegisterMspCallbacks(BSP_I2C_Cb_t *Callback)
{
  ...
  if (HAL_I2C_RegisterCallback(&hbus_i2c6, HAL_I2C_MSPINIT_CB_ID, Callback->pMspI2cInitCb) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else if (HAL_I2C_RegisterCallback(&hbus_i2c6, HAL_I2C_MSPDEINIT_CB_ID, Callback->pMspI2cDeInitCb) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else
...
static void I2C6_MspInit(I2C_HandleTypeDef *hI2c)
{
  ...
  BUS_I2C6_SCL_GPIO_CLK_ENABLE();
  /* Enable SDA GPIO clock */
  BUS_I2C6_SDA_GPIO_CLK_ENABLE();
  ...
  HAL_GPIO_Init(BUS_I2C6_SCL_GPIO_PORT, &gpio_init_structure);
  ...
  HAL_GPIO_Init(BUS_I2C6_SDA_GPIO_PORT, &gpio_init_structure);
  ...
  BUS_I2C6_FORCE_RESET();

  /* Release the I2C peripheral clock reset */
  BUS_I2C6_RELEASE_RESET();
}
...
static void I2C6_MspDeInit(I2C_HandleTypeDef *hI2c)
{
  ...
  gpio_init_structure.Pin = BUS_I2C6_SCL_PIN;
  HAL_GPIO_DeInit(BUS_I2C6_SCL_GPIO_PORT, gpio_init_structure.Pin);
  gpio_init_structure.Pin = BUS_I2C6_SDA_PIN;
  HAL_GPIO_DeInit(BUS_I2C6_SDA_GPIO_PORT, gpio_init_structure.Pin);

  /* Disable I2C clock */
  BUS_I2C6_CLK_DISABLE();
}
...
/**
  ...
  * @retval BSP status
  */
static int32_t I2C6_WriteReg(uint16_t DevAddr, uint16_t Reg, uint16_t MemAddSize, uint8_t *pData, uint16_t Length)
{
  if (HAL_I2C_Mem_Write(&hbus_i2c6, DevAddr, Reg, MemAddSize, pData, Length, 10000) == HAL_OK)
  {
...
/**
  ...
  * @retval BSP status
  */
static int32_t I2C6_ReadReg(uint16_t DevAddr, uint16_t Reg, uint16_t MemAddSize, uint8_t *pData, uint16_t Length)
{
  if (HAL_I2C_Mem_Read(&hbus_i2c6, DevAddr, Reg, MemAddSize, pData, Length, 10000) == HAL_OK)
  {
    return BSP_ERROR_NONE;
  }
...
static int32_t I2C6_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  if (HAL_I2C_Master_Receive(&hbus_i2c6, DevAddr, pData, Length, 10000) == HAL_OK)
  {
    return BSP_ERROR_NONE;
  }
...
static int32_t I2C6_Send(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  if (HAL_I2C_Master_Transmit(&hbus_i2c6, DevAddr, pData, Length, 10000) == HAL_OK)
  {
    return BSP_ERROR_NONE;
  }
