...
int16_t sensirion_measure_ticks(void* i2c_bus, uint8_t i2c_address, uint8_t command, 
    ...
    error = sensirion_i2c_write_data(i2c_bus, i2c_address, buffer, offset);
    if (error) {
        return error;
    }
    ...
    error = sensirion_i2c_read_data_inplace(i2c_bus, i2c_address, buffer, read_bytes);
    return error;
}
...
int16_t sensirion_read_serial_number(void* i2c_bus, uint8_t i2c_address, uint32_t* serial_number) {
    ...
    error = sensirion_i2c_read_data_inplace(i2c_bus, i2c_address, buffer, 4);
    if (error) {
        return error;
    }
